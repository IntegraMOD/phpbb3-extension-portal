...
class vars_module
{
    ...
    public function main($module_id, $mode)
    {
        global $db, $user, $auth, $template, $cache, $request;
        global $config, $phpbb_root_path, $phpbb_admin_path, $phpEx, $k_config, $table_prefix, $sgp_functions_admin;
        ...
        if (isset($block))
        {
			...
			$title = isset($row['title']) ? str_replace(' ', '_', strtoupper($row['title'])) : '';

			$block_id = isset($row['id']) ? $row['id'] : null;
			$var_file_name = isset($row['var_file_name']) ? $row['var_file_name'] : null;
        ...
        // switch to other var setups 11 March 2010
        if ($switch)
        {
